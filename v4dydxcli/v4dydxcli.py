...
sys.path.insert(
    0, os.path.dirname(os.path.abspath(__file__)) + "/v4-clients/v4-client-py"
)
...
def getticksize():
    count = 0
    try:
        r = get(url=INDEXERURL + "/perpetualMarkets", params={"ticker": dydxmarket})
        r.raise_for_status()
        if r.status_code == 200:
...
def findordera():
    ...
    for subaccountnumber in subaccountlist:
        ...
        while newheight < height:
            ...
            try:
                if (
                    clientid == constants.ORDER_STATUS_OPEN
                    or clientid == constants.ORDER_STATUS_FILLED
                    or clientid == constants.ORDER_STATUS_CANCELED
                    or clientid == constants.ORDER_STATUS_UNTRIGGERED
                ):
                    get_subaccount_orders_result = (
                        client.indexer_client.account.get_subaccount_orders(
                ...
                if len(get_subaccount_orders_result.data) > 0:
                    if len(get_subaccount_orders_result.data) > 1:
                        topheight = get_subaccount_orders_result.data[0][
                            "createdAtHeight"
                        ]
                        newheight = get_subaccount_orders_result.data[-1][
                            "createdAtHeight"
                        ]
                        if topheight == newheight:
                            print(
                                "Error: more than 100 records with the same createdAtHeight",
                                topheight,
                            )
                ...
                print(
                    datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "findordera() api query failed (%s)" % error,
                )
                return None
        ...
        while newheight < height:
            ...
            try:
                if (
                    ...
                    get_subaccount_orders_result = (
                        client.indexer_client.account.get_subaccount_orders(
                            address=walletaddress,
                            subaccount_number=subaccountnumber,
                            good_til_block_time_before_or_at=datetime.utcfromtimestamp(
                                height
                            ).isoformat()[:-3]
                            + "Z",
                            return_latest_orders=True,
                ...
                if len(get_subaccount_orders_result.data) > 0:
                    if len(get_subaccount_orders_result.data) > 1:
                        topheight = isoparse(
                            get_subaccount_orders_result.data[0]["goodTilBlockTime"]
                        ).timestamp()
                        newheight = isoparse(
                            get_subaccount_orders_result.data[-1]["goodTilBlockTime"]
                        ).timestamp()
                        if topheight == newheight:
                            print(
                                "Error: more than 100 records with the same goodTilBlockTime",
                                topheight,
                            )
                            # move to next subaccount
                            break
                ...
                print(
                    datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "findorderb() api query failed (%s)" % error,
                )
                return None
    ...
    for subaccountnumber in subaccountlist:
        ...
        while newheight < height:
            ...
            try:
                ...
                print(
                    datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "findorder2a() api query failed (%s)" % error,
                )
                return None
            ...
            if (
                ...
                r = get(
                    ...
                    params={
                        ...
                        "subaccountNumber": subaccountnumber,
                        "return_latest_orders": True,
                        "goodTilBlockTimeBeforeOrAt": datetime.utcfromtimestamp(
                            height
                        ).isoformat()[:-3]
                        + "Z",
                        "status": clientid,
            ...
            try:
                ...
                if r.status_code == 200:
                    if len(r.json()) > 0:
                        if len(r.json()) > 1:
                            topheight = isoparse(
                                r.json()[0]["goodTilBlockTime"]
                            ).timestamp()
                            newheight = isoparse(
                                r.json()[-1]["goodTilBlockTime"]
                            ).timestamp()
                            if topheight == newheight:
                                print(
                                    "Error: more than 100 records with the same goodTilBlockTime",
                ...
                print(
                    datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "findorder2b() api query failed (%s)" % error,
                )
                return None
...
def getbalance():
    try:
        subaccounts_response = client.indexer_client.account.get_subaccounts(
            walletaddress
        )
        for item in subaccounts_response.data["subaccounts"]:
            if subaccount == "all":
                print(
                    item["address"] + ":" + str(item["subaccountNumber"]),
                    item["equity"],
                )
                        ...
                        if r.status_code == 200:
                            if len(r.json()["positions"]) > 0:
                                if len(r.json()["positions"]) == 100:
                                    topheight = r.json()["positions"][0][
                                        "createdAtHeight"
                                    ]
                                    newheight = r.json()["positions"][-1][
                                        "createdAtHeight"
                                    ]
                                    if topheight == newheight:
                                        print(
                                ...
                                for item in r.json()["positions"]:
                                    print(
                                        walletaddress + ":" + str(subaccountnumber),
                                        item["market"].ljust(9),
                                        item["size"],
                                    )
            ...
            if r.status_code == 200:
                ...
                while int(newheight) < int(height):
                    ...
                    try:
                        ...
                        if (
                            len(
                                get_subaccount_perpetual_positions_result.data[
                                    "positions"
                                ]
                            )
                            > 0
                            ...
                            if (
                                ...
                                if topheight == newheight:
                                    ...
                                    # move to next subaccount
                                    break
                            for item in get_subaccount_perpetual_positions_result.data[
                                "positions"
                            ]:
                                print(
                                    walletaddress + ":" + str(subaccountnumber),
    ...
    while counter2 < 10:
        try:
            ...
            if (
                ...
                or orderstatus == constants.ORDER_STATUS_FILLED
                or orderstatus == constants.ORDER_STATUS_CANCELED
                or orderstatus == constants.ORDER_STATUS_UNTRIGGERED
            ):
                print(orderstatus)
                break
            print(
                "Waiting 1 second for order to be OPEN, FILLED, CANCELED, or UNTRIGGERED."
            )
            time.sleep(1)
        except Exception as error:
...
def checkcancel():
    ...
    while counter2 < 10:
        try:
            ...
            if (
                orderstatus == constants.ORDER_STATUS_FILLED
                or orderstatus == constants.ORDER_STATUS_CANCELED
            ):
                print(orderstatus)
                break
...
if command == "balance":
    ...
    if simpleorderbook == "y":
        ...
        bestbid = os.popen(
            ...
            + "/"
            + dydxmarket
            + "/bids; grep \"\" /dev/null * 2>> /dev/null | sed 's/:/ /' | sort -n | tail -1 | awk '{print $1}'"
        ).read()[:-1]
        bestask = os.popen(
            "cd "
            + ramdiskpath
            + "/"
            + dydxmarket
            + "/asks; grep \"\" /dev/null * 2>> /dev/null | sed 's/:/ /' | sort -n | head -1 | awk '{print $1}'"
        ).read()[:-1]
    ...
    clientid = randrange(
        0, 2**31 - 1
    )  # random number between 0 and max(int32) inclusive
    ...
    if simpleorderbook == "y":
        ...
        dydxticksize = os.popen(
            "tail -1 " + ramdiskpath + "/v4dydxmarketdata/" + dydxmarket + "/tickSize"
        ).read()[:-1]
    ...
    clientid = randrange(
        0, 999999999
    )  # random number between 0 and 999,999,999 inclusive
    latest_block_result = client.validator_client.get.latest_block()
    good_til_block_value = latest_block_result.block.header.height + 1 + 10
    print(
        dydxmarket,
    ...
    if simpleorderbook == "y":
        ...
        dydxstepsize = os.popen(
            "tail -1 " + ramdiskpath + "/v4dydxmarketdata/" + dydxmarket + "/stepSize"
        ).read()[:-1]
    ...
    if len(argv) < 4:
        ...
        print(
            "clientid can also be a status such as OPEN, FILLED, CANCELED, or UNTRIGGERED"
        )
        exit()
    elif len(argv) > 4:
        searchtype = argv[3]
    ...
    for order in orderlist:
        ...
        orderexpiration = int(
            isoparse(order["goodTilBlockTime"]).timestamp() - time.time() + 60
        )
        cancelorder()
        checkcancel()
else:
    print(
        "Available commands: balance, positions, buyquantity, sellquantity, buyusdc, sellusdc, getorder, getorderid, buyquantitylimit, sellquantitylimit, cancelorder"
    )
